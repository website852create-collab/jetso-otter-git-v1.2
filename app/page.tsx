...
export default async function Home() {
  // 直接從 View 讀取，不需要再濾 is_active，View 已經濾好了
  ...
  if (error) {
    ...
    return <div className="p-8 text-center">載入失敗，請稍後再試。</div>;
  }
    ...
    <div className="space-y-8">
      ...
      {categories.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {categories.map((cat) => (
            <span key={cat} className="px-4 py-1 bg-white border border-otter-200 rounded-full text-sm text-otter-600 whitespace-nowrap">
              {cat}
            </span>
          ))}
        </div>
